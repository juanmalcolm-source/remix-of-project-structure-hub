...
import {
  BarChart3, Users, UserCircle, Film, Megaphone, Landmark,
  LogOut, Save, Settings, Palette, Clapperboard, Wallet, Menu,
} from 'lucide-react';
  ...
  const renderSidebarContent = () => (
    <>
      ...
      <div className="px-4 py-4 border-b border-sidebar-border">
        <Button variant="ghost" size="sm" onClick={() => navigateTo('/proyectos')} className="text-sidebar-foreground/70 hover:text-sidebar-foreground hover:bg-sidebar-accent mb-2 -ml-2">
          ← Proyectos
        </Button>
      ...
      <nav className="flex-1 py-4 px-3 space-y-1 overflow-y-auto">
        {tabs.map((tab) => {
          ...
          return (
            <button key={tab.id} onClick={() => navigateTo(tab.path)} className={cn('sidebar-link w-full text-left', isActive && 'sidebar-link-active bg-purple-500/10 text-purple-600')}>
      ...
      <div className="p-4 border-t border-sidebar-border">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm text-sidebar-foreground truncate">{profile?.company_name || user?.email?.split('@')[0]}</p>
          </div>
          <ThemeToggle />
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="lg:ml-64">
        <div className="hidden lg:flex items-center justify-between px-8 py-4 border-b bg-card/50">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem><BreadcrumbPage>Audiencias</BreadcrumbPage></BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        ...
        <div className="p-6 lg:p-8 animate-in fade-in-0 slide-in-from-bottom-2 duration-300">
          <ExpertChatWrapper projectId={projectId!} expertType="audiencias">
            {children}
          </ExpertChatWrapper>
        </div>
      </main>
