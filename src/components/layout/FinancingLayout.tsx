...
import {
  Settings, Percent, Map, Wallet, FlaskConical, Calendar,
  LogOut, Save, Palette, Clapperboard, TrendingUp, Megaphone, Landmark, Menu,
} from 'lucide-react';
    ...
    <>
      ...
      <nav className="flex-1 py-4 px-3 space-y-1 overflow-y-auto">
        {financingTabs.map((tab) => {
          ...
          return (
            <button key={tab.id} onClick={() => navigateTo(tab.path)} className={cn('sidebar-link w-full text-left', isActive && 'sidebar-link-active bg-green-500/10 text-green-600')}>
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="lg:ml-64">
        <div className="hidden lg:flex items-center justify-between px-8 py-4 border-b bg-card/50">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem><BreadcrumbPage>Financiación</BreadcrumbPage></BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        ...
        <div className="p-6 lg:p-8 animate-in fade-in-0 slide-in-from-bottom-2 duration-300">
          <ExpertChatWrapper projectId={projectId!} expertType="financiacion">
            {children}
          </ExpertChatWrapper>
        </div>
      </main>
