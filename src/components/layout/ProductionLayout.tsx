...
import {
  Users, MapPin, Calendar, Calculator, FileSpreadsheet, FileText,
  LogOut, Settings, Save, Palette, Wallet, Clapperboard, Navigation, Megaphone, Landmark, Menu,
} from 'lucide-react';
    ...
    <>
      ...
      <nav className="flex-1 py-4 px-3 space-y-1 overflow-y-auto">
        {productionTabs.map((tab) => {
          ...
          return (
            <button key={tab.id} onClick={() => navigateTo(tab.path)} className={cn('sidebar-link w-full text-left', isActive && 'sidebar-link-active bg-orange-500/10 text-orange-600')}>
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="lg:ml-64">
        <div className="hidden lg:flex items-center justify-between px-8 py-4 border-b bg-card/50">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem><BreadcrumbPage>Producción</BreadcrumbPage></BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        ...
        <div className="p-6 lg:p-8 animate-in fade-in-0 slide-in-from-bottom-2 duration-300">
          <ExpertChatWrapper projectId={projectId!} expertType="produccion">
            {children}
          </ExpertChatWrapper>
        </div>
      </main>
