...
export async function createProjectFromAnalysis(
  ...
  const analysisData: Record<string, unknown> = {
    ...
    strengths: analisis.analisis_dafo?.fortalezas?.map(f => f.descripcion) || [
    ...
    weaknesses: analisis.analisis_dafo?.debilidades?.map(d => d.descripcion) || [],
    improvement_suggestions: analisis.recomendaciones_estrategicas?.sugerencias_desarrollo || [],
    viability_factors_positive: analisis.recomendaciones_estrategicas?.factores_positivos || [],
    viability_factors_negative: analisis.recomendaciones_estrategicas?.factores_negativos || [],
